{
	std::ofstream fout("bias.dat");

	fout << "qSq bin\tmoment\tvalue\t\terror\t\terror (toy)\t\tbias1\t\tbias2" << std::endl;

	for(Int_t q=0; q<19; ++q) {
		TString toyName("toy/"); toyName+=q; toyName+="_results.root";
