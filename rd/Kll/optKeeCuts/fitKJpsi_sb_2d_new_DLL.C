{
	...
	RooDataSet * data = new RooDataSet("data", "dataset with B_REFITTED_M", DecayTree, RooArgSet(B_M,BDT_KeeBig,K_Kst_ProbNNk,e_plus_PIDe,e_minus_PIDe,K_Kst_PIDe));
	...
	for(Int_t i=0; i<ni; ++i) {
		for(Int_t j=0; j<nj; ++j) {
			...
			cutStr+= " && e_plus_PIDe>";  cutStr+= cute;
			cutStr+= " && e_minus_PIDe>"; cutStr+= cute;
