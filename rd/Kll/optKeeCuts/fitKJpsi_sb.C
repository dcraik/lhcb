{
	...
	//2
	Double_t bdtMin(-0.1), bdtMax(0.0);
	Double_t kMin(0.1), kMax(0.2);
	Double_t eMin(0.0), eMax(0.1);
	Int_t ni(6), nj(6), nk(6);
...
//				Double_t nsig = DecayTree->GetEntries(cutStr+"&&mKJpsi>5200&&mKJpsi<5400");
				...
				B[i][j][k] = nbkgSB.getVal()*fComb2/fComb5;
				S[i][j][k] = nsig - B[i][j][k];

				std::cout << S[i][j][k] << "\t" << B[i][j][k] << std::endl;
