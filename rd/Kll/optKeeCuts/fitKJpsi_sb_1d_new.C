{
	...
	DecayTree->SetBranchStatus("e_plus_PIDe",1);
	DecayTree->SetBranchStatus("e_minus_PIDe",1);
	DecayTree->SetBranchStatus("category",1);

	//B_M 
	RooRealVar B_M("B_plus_M","; m(K J/psi) (MeV); Candidates / 10 MeV", 5200,6200);
	...
	RooRealVar e_plus_PIDe("e_plus_PIDe","",-100.,100.);
	RooRealVar e_minus_PIDe("e_minus_PIDe","",-100.,100.);

	RooDataSet * data = new RooDataSet("data", "dataset with B_REFITTED_M", DecayTree, RooArgSet(B_M,BDT_KeeBig,K_Kst_ProbNNk,e_plus_ProbNNe,e_minus_ProbNNe,K_Kst_PIDe,e_plus_PIDe,e_minus_PIDe));
	...
	Double_t bdtMin(-0.3), bdtMax(0.7);
	Int_t ni(101);
	...
	for(Int_t i=0; i<ni; ++i) {
			...
			std::cout << cutbdt << std::endl;

			cutStr = "K_Kst_ProbNNk>0.2 && K_Kst_PIDe<0 && e_plus_PIDe>0 && e_minus_PIDe>0 && BDTKeeBig>"; cutStr+= cutbdt;
...
//			Double_t nsig = DecayTree->GetEntries(cutStr+"&&B_plus_M>5200&&B_plus_M<5400");
			Double_t nsig = DecayTree->GetEntries(cutStr+"&&B_plus_M>5265&&B_plus_M<5305&&mKJpsi>5200&&mKJpsi<5400");
			...
			B[i] = nbkgSB.getVal()*fComb2/fComb5;
			S[i] = nsig;
