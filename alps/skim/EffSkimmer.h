...
//struct to store gen indices of decay
struct DecayIndices {
	int b{-1};
	int x{-1};
	int k{-1};
...
class EffSkimmer {
	public :
		...
		Gen* addGen(TTree* tree, TString pre);
		...
		DecayIndices* checkTrue_Kpi_pipineu(int ib,int ix,int ik,int ipi,int ixpip,int ixpim,int ineu,int neuPID);
		DecayIndices* findTrueDecay_Kpi_pipineu(int igb);
		DecayIndices* findRecoDecay_Kpi_pipineu(DecayIndices* ig);

		void fillGen(Gen* part, int idx);
...
#ifdef EffSkimmer_cxx
EffSkimmer::EffSkimmer(int dir) : fChain(0) 
{
...
EffSkimmer::~EffSkimmer()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t EffSkimmer::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t EffSkimmer::LoadTree(Long64_t entry)
{
...
void EffSkimmer::Init(TTree *tree)
{
...
void EffSkimmer::Show(Long64_t entry)
{
...
Int_t EffSkimmer::Cut(Long64_t entry)
{
