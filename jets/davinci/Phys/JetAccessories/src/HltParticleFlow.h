...
private:
  ...
  template<class Object> bool retrieve(Object &obj, const Input &in) {
    ...
    else {obj = 0; warning() << "retrieve(): Could not retrieve " 
			     << in.name << "s from " 
  ...
  // Unique VELO segment map and key creator.
  unordered_map<double, const Track*> m_vels;
  double veloKey(const Track *trk) {
    double key = 1;
    for (vector<LHCbID>::const_iterator id = trk->lhcbIDs().begin(); 
	 id != trk->lhcbIDs().end(); ++id)
     if (id->isVelo()) key *= (double(id->veloID().channelID())/1000000.0);
    return key;
  }
