...
: GaudiHistoAlg(name , svc), m_prtSvc(0), m_prtSta(0), m_prtVrt(0), m_inNow(0) {
  ...
  declareProperty("Inputs", m_inLocs,
		  ...
		  "<location>].");
  declareProperty("Output", m_outLoc = "Phys/ParticleFlow/Particles",
...
StatusCode HltParticleFlow::execute() {
  ...
  // Build the unique VELO segments map.
  if (exist<ProtoParticles>("Rec/ProtoP/Charged")) {
    ProtoParticles *pros = get<ProtoParticles>("Rec/ProtoP/Charged");
    for (ProtoParticles::iterator pro = pros->begin(); pro != pros->end();
	 ++pro) {
      const Track *trk = (*pro)->track();
      if (!trk) continue;
      double key = veloKey(trk);
      if (key == 1) continue;
      unordered_map<double, const Track*>::iterator itr = m_vels.find(key);
      if (itr == m_vels.end()) m_vels[key] = trk;
      else if (trk->ghostProbability() < itr->second->ghostProbability())
	itr->second = trk;
    }
  }
    ...
    if (inPro != m_inPros.end() && inLoc == inPro->index) {
      ...
      if (exist<ProtoParticles>(inPro->loc)) {
        ...
        for (ProtoParticles::iterator pro = pros->begin(); pro != pros->end();
          ...
          warning() << "execute(): Could not retrieve " << inPro->name 
		    << " from " << inPro->loc << endmsg;
      }
      ++inPro;
      continue;
    }
...
void HltParticleFlow::add(const Particle *prt, const Input &in) {
  ...
  for (Particle::ConstVector::iterator dtr = dtrs.begin(); dtr != dtrs.end()
	 && !in.dtrs; ++dtr) if (used((*dtr)->proto())) return;
  for (Particle::ConstVector::iterator dtr = dtrs.begin(); dtr != dtrs.end();
       ++dtr) {
    if (in.dtrs && !used((*dtr)->proto())) create(*dtr); 
...
Particle *HltParticleFlow::create(const ProtoParticle *pro, const int &id,
				  const double &m) {
  Particle *prt(0); Gaudi::LorentzVector vec(0, 0, 0, 0);
  ...
  if (pro->track()) {
    ...
    if (type == Track::Velo) p = m_trkVlP;
      ...
      if (e != 0);
      else if (type == Track::Long && c/e < m_trkLnErrMax) p = m_trkLnP;
      else if (type == Track::Upstream && c/e < m_trkUpErrMax) p = m_trkUpP;
      else if (type == Track::Downstream && c/e < m_trkDnErrMax) p = m_trkDnP;
    }
  ...
  if (vec != vec) {delete prt; return 0;}
  vec.SetE(sqrt(vec.P2() + m*m));
  prt->setMomentum(vec); prt->setMeasuredMass(m); prt->setMeasuredMassErr(0);
  prt->setProto(pro); 
...
void HltParticleFlow::use(const Track *trk, const Particle *prt) {
  m_trks.insert(trk); m_vels[veloKey(trk)] = trk;
...
bool HltParticleFlow::used(const Track *trk) {
  if (m_inNow && !m_inNow->dtrs) {
    if (trk->pt() > 1e6) return true;
    if (trk->ghostProbability() > 0.1) return true;
  }
  if (m_trks.find(trk) != m_trks.end()) return true;
  unordered_map<double, const Track*>::iterator itr = 
    m_vels.find(veloKey(trk));
  if (itr != m_vels.end()) {
    if (m_inNow && !m_inNow->dtrs && itr->second != trk) return true;
    if (m_trks.find(itr->second) != m_trks.end()) return true;
  } return false;}
