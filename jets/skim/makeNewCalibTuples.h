...
#ifndef makeNewCalibTuples_h
#define makeNewCalibTuples_h
...
class makeNewCalibTuples {
	public :
		...
		makeNewCalibTuples(int year, int part=-1);
		virtual ~makeNewCalibTuples();
		...
		//bool checkTruth(int idxD, int needPID, int needPi, int needK, int needP, int needMu, int needOppPi=-1, int needOppK=-1, int needOppP=-1);
...
//	if(year==5) {
//		boost::progress_display show_addfile_progress( 100 );
//		for(int i=0; i<100; ++i) {
...
//		}
//	} else if(year==15) {
//		boost::progress_display show_addfile_progress( 1055 );
...
//		}
//	} else if(year==2017) {
//		boost::progress_display show_addfile_progress( 165+412 );
//		for(int i=0; i<165; ++i) {
//			++show_addfile_progress;
//			sprintf(str,"/eos/lhcb/user/d/dcraik/jets/372/%d/output.root",i);
//			if(gSystem->AccessPathName(str)) continue;
//			t->Add(str);
//		}
//		for(int i=0; i<412; ++i) {
//			++show_addfile_progress;
//			sprintf(str,"/eos/lhcb/user/d/dcraik/jets/373/%d/output.root",i);
//			if(gSystem->AccessPathName(str)) continue;
//			t->Add(str);
//		}
	...
	if(year==100) {
		...
		minTruePt_=0.;
		maxTruePt_=200000.;
		boost::progress_display show_addfile_progress( 488 );
		for(int i=0; i<488; ++i) {
			++show_addfile_progress;
...
makeNewCalibTuples::~makeNewCalibTuples()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t makeNewCalibTuples::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t makeNewCalibTuples::LoadTree(Long64_t entry)
{
...
void makeNewCalibTuples::Init(TTree *tree)
{
	...
	TString fname = "/tmp/dcraik/for_yandex_data_new_"; fname+=year_;
	if(part_>=0 && part_<10) {
		fname+="_";
		fname+=part_;
	}
	fname+=".root";
	//TString fname = "/tmp/dcraik/for_yandex_data_new_"; fname+=year_; fname+="_newRun.root";
...
Bool_t makeNewCalibTuples::Notify()
{
...
void makeNewCalibTuples::Show(Long64_t entry)
{
...
Int_t makeNewCalibTuples::Cut(Long64_t entry)
{
...
void makeNewCalibTuples::clearEventOutputs() {
	EVT = 0.;
	DEC = 0.;
	NPV = 0.;
	clearOutputs();
...
void makeNewCalibTuples::clearOutputs() {
	JPX = 0.;
	JPY = 0.;
	JPZ = 0.;
	JE = 0.;
...
void makeNewCalibTuples::clearOutputVectors() {
	SVM        ->clear();
	SVMCOR     ->clear();
	SVMCORERR  ->clear();
	SVMINPERP  ->clear();
...
#endif // #ifdef makeNewCalibTuples_cxx
