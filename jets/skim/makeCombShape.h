...
#ifndef makeCombShape_h
#define makeCombShape_h
...
#ifdef makeCombShape_cxx
makeCombShape::makeCombShape() : fChain(0)
{
	...
	fout_ = TFile::Open("combHist.root","RECREATE");
	hist_ = new TH2D("comb","",20,1785.,1945.,20,-5,15);
}

makeCombShape::~makeCombShape()
{
	if (!fChain) return;
	delete fChain->GetCurrentFile();
}

Int_t makeCombShape::GetEntry(Long64_t entry)
{
	// Read contents of entry.
	if (!fChain) return 0;
	return fChain->GetEntry(entry);
}
Long64_t makeCombShape::LoadTree(Long64_t entry)
{
...
void makeCombShape::Init(TTree *tree)
{
...
void makeCombShape::Show(Long64_t entry)
{
...
Int_t makeCombShape::Cut(Long64_t entry)
{
