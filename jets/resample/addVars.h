...
#ifdef addVars_cxx
addVars::addVars(TString name) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   
   TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject(name+".root");
   TTree *tree(0);
   if (!f || !f->IsOpen()) {
      f = new TFile(name+".root");
   }
   f->GetObject("data",tree);

   Init(tree);

   newfile = TFile::Open(name+"_addVars.root","RECREATE");
...
Int_t addVars::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t addVars::LoadTree(Long64_t entry)
{
...
void addVars::Show(Long64_t entry)
{
...
Int_t addVars::Cut(Long64_t entry)
{
