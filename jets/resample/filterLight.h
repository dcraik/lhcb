...
//   if (tree == 0) {
//      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/tmp/dcraik/light_new.root");
//      if (!f || !f->IsOpen()) {
//         f = new TFile("/tmp/dcraik/light_new.root");
//      }
...
//   }
   ...
   std::ifstream fin;
   fin.open("tuplesLight.log", std::fstream::in);
   while(fin.good()) {
      TString fname;
      fin >> fname;
      if(fname.Length()>0) {
   ...
   newfile = TFile::Open("/tmp/dcraik/lightjets_filtered.root","RECREATE");
