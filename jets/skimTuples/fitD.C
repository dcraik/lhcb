...
void fit(){
	...
	RooRealVar DM(  "DM",  "DM",  1790.,  1950.,  "MeV/#it{c}^{2}"); 
	DM.setBins(30);
	RooRealVar DLOGIPCHI2(  "DLOGIPCHI2",  "DLOGIPCHI2",  -5.,  15.,  ""); 
	DLOGIPCHI2.setBins(20);

	RooRealVar dMass("dMass","mass mean",1870.,1850.,1890.);
	...
	RooRealVar DE(  "DE",  "DE",  0.,   2e6); 
	RooRealVar DPX( "DPX", "DPX", -1e5, 1e5); 
	RooRealVar DPY( "DPY", "DPY", -1e5, 1e5); 
	RooRealVar DPZ( "DPZ", "DPZ", 0.,   2e6); 
	...
	RooDataSet* dsPeak = dynamic_cast<RooDataSet*>(ds.reduce("DM>1850 && DM<1890"));
	RooDataSet* dsSB   = dynamic_cast<RooDataSet*>(ds.reduce("DM<1810 || DM>1930"));
	...
	p0.setConstant();
        ...
        DM.setRange("signal",1850.,1890.);
        DM.setRange("sideLo",1790.,1810.);
        DM.setRange("sideHi",1930.,1950.);
        DM.setRange("full",1790.,1950.);
	...
	//RooRealVar promptWidthL("promptWidthL","width prompt",1.,0.5,5.);
	//RooRealVar promptWidthR("promptWidthR","width prompt",1.,0.5,5.);
	//RooBifurGauss promptLOGIPCHI2("promptLOGIPCHI2","",DLOGIPCHI2,promptMean,promptWidthL,promptWidthR);
	...
	//promptWidthL.setConstant();
	//promptWidthR.setConstant();
	...
	plot(DM, 1790., 1950., ds, data_pdf, sig_pdfs, bkg_pdfs, "DM_full", "m_{K#pi#pi}");
	plot(DLOGIPCHI2, -5., 15., *dsPeak, data_pdf2, sig_pdfs2, bkg_pdfs2, "DIPChi2_full", "log(IP#chi^{2})");
	...
	//params.add(promptWidthL);
	//params.add(promptWidthR);
	params.add(displacedMean);
	params.add(displacedWidth);
	print("Dparams_full.dat",params);
	...
	RooAddPdf data_pdf2SP( "data_pdf2SP",  "data_pdf2SP", RooArgList(promptLOGIPCHI2,displacedLOGIPCHI2,bkgLOGIPCHI2), RooArgList(promptYieldSP,displacedYieldSP,bkgYieldSP) );
        ...
        TFile * newFile0 = TFile::Open("data0_Dp_sWeights.root","RECREATE");
	...
	RooDataSet* ds4Peak = dynamic_cast<RooDataSet*>(ds4.reduce("DM>1850 && DM<1890"));
	...
	p0.setConstant();
        ...
        fsig_1 = sigMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("signal"))->getVal();
        fsig_2 = sigMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("sideLo"))->getVal();
        fsig_3 = sigMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("sideHi"))->getVal();
        fsig_0 = sigMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("full"))->getVal();

        fbkg_1 = bkgMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("signal"))->getVal();
        fbkg_2 = bkgMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("sideLo"))->getVal();
        fbkg_3 = bkgMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("sideHi"))->getVal();
        fbkg_0 = bkgMass.createIntegral(RooArgSet(DM),RooFit::NormSet(DM),RooFit::Range("full"))->getVal();
	...
	plot(DM, 1790., 1950., ds4, data_pdf, sig_pdfs, bkg_pdfs, "DM_4", "m_{K#pi#pi}");
	plot(DLOGIPCHI2, -5., 15., *ds4Peak, data_pdf2, sig_pdfs2, bkg_pdfs2, "DIPChi2_4", "log(IP#chi^{2})");

	print("Dparams_4.dat",params);
	...
	bkgYieldSP.setVal(bkgInPeakYield.getVal());
        ...
        TFile * newFile1 = TFile::Open("data4_Dp_sWeights.root","RECREATE");
	...
	RooDataSet* ds5Peak = dynamic_cast<RooDataSet*>(ds5.reduce("DM>1850 && DM<1890"));
	...
	plot(DM, 1790., 1950., ds5, data_pdf, sig_pdfs, bkg_pdfs, "DM_5", "m_{K#pi#pi}");
	plot(DLOGIPCHI2, -5., 15., *ds5Peak, data_pdf2, sig_pdfs2, bkg_pdfs2, "DIPChi2_5", "log(IP#chi^{2})");

	print("Dparams_5.dat",params);
	...
	bkgYieldSP.setVal(bkgInPeakYield.getVal());
        ...
        TFile * newFile2 = TFile::Open("data5_Dp_sWeights.root","RECREATE");
