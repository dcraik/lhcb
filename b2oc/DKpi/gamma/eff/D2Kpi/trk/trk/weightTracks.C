{
  ...
  TFile* dataFile    = TFile::Open("/data/lhcb/phrkbf/B2DKpi/Job2012_B2DKpi_D2Kpi_Scaled/B2D0Kpi_D02Kpi_selBd_Dsignal_Dst25_vetoes_PID3_NND2Kpi_addIMs.root");
  TFile* mcFile      = TFile::Open("/data/lhcb/phrkbf/B2DKpi/JobSim08aMC/D2Kpi/Bd2D0Kpi/D2Kpi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2Kpi_addIMs_addMisIDIMs.root");
  ...
  Int_t pi_ID(0.), D0pi_ID(0.), nTracks(0.);
  ...
  dataTree->SetBranchAddress("D0pi_ID",        &D0pi_ID       );
  ...
  mcTree->SetBranchAddress(  "D0pi_ID",        &D0pi_ID       );
  ...
  mcTree->SetBranchAddress("D0K_TRUEID",              &D0K_TRUEID);
  mcTree->SetBranchAddress("D0pi_TRUEID",             &D0pi_TRUEID);
  mcTree->SetBranchAddress("K_TRUEID",                &K_TRUEID);
  mcTree->SetBranchAddress("pi_TRUEID",               &pi_TRUEID);
  mcTree->SetBranchAddress("D0K_MC_GD_MOTHER_KEY",    &D0K_MC_GD_MOTHER_KEY);
  mcTree->SetBranchAddress("D0pi_MC_GD_MOTHER_KEY",   &D0pi_MC_GD_MOTHER_KEY);
  mcTree->SetBranchAddress("K_MC_MOTHER_KEY",         &K_MC_MOTHER_KEY);
  mcTree->SetBranchAddress("pi_MC_MOTHER_KEY",        &pi_MC_MOTHER_KEY);

  //determine optimal p binning
  pBins[0] = 0;
  pBins[nBinsP] = 200000.;
  ...
  //determine optimal nTracks binning
  multBins[0] = 0;
  ...
  //determine weights to match (p,eta,nTracks) to data for each of the 4 tracks individually
  ...
  for(Int_t i=0; i<nEntriesData; ++i) {
    ...
    if(D0pi_ID>0) {
      dataHist3->Fill(B_D0_D0p_CM_P,0.5*log((B_D0_D0p_CM_P+B_D0_D0p_CM_PZ)/(B_D0_D0p_CM_P-B_D0_D0p_CM_PZ)),nTracks);
      dataHist4->Fill(B_D0_D0m_CM_P,0.5*log((B_D0_D0m_CM_P+B_D0_D0m_CM_PZ)/(B_D0_D0m_CM_P-B_D0_D0m_CM_PZ)),nTracks);
  ...
  for(Int_t i=0; i<nEntriesMC; ++i) {
    ...
    if(abs(B_TRUEID)!=511||abs(D_TRUEID)!=421||abs(K_TRUEID)!=321||abs(pi_TRUEID)!=211||abs(D0K_TRUEID)!=321||abs(D0pi_TRUEID)!=211) continue;
    if(K_MC_MOTHER_KEY!=pi_MC_MOTHER_KEY||K_MC_MOTHER_KEY!=D0K_MC_GD_MOTHER_KEY||K_MC_MOTHER_KEY!=D0pi_MC_GD_MOTHER_KEY) continue;
    ...
    if(D0pi_ID>0) {
      mcHist3->Fill(B_D0_D0p_CM_P,0.5*log((B_D0_D0p_CM_P+B_D0_D0p_CM_PZ)/(B_D0_D0p_CM_P-B_D0_D0p_CM_PZ)),nTracks);
      mcHist4->Fill(B_D0_D0m_CM_P,0.5*log((B_D0_D0m_CM_P+B_D0_D0m_CM_PZ)/(B_D0_D0m_CM_P-B_D0_D0m_CM_PZ)),nTracks);
  ...
  Int_t piBin(0.), KBin(0.), D0piBin(0.), D0KBin(0.);

  //now write out the weights into a tuple
  TFile * newFile = TFile::Open("D2Kpi_Bd2D0Kpi_trackWeight.root","recreate");
  TTree * newTree = mcTree->CloneTree(0);

  newTree->Branch("piWeight",      &piWeight,      "piWeight/D");
  newTree->Branch("KWeight",       &KWeight,       "KWeight/D");
  newTree->Branch("D0piWeight",    &D0piWeight,    "D0piWeight/D");
  newTree->Branch("D0KWeight",     &D0KWeight,     "D0KWeight/D");
  newTree->Branch("piWeightErr",   &piWeightErr,   "piWeightErr/D");
  newTree->Branch("KWeightErr",    &KWeightErr,    "KWeightErr/D");
  newTree->Branch("D0piWeightErr", &D0piWeightErr, "D0piWeightErr/D");
  newTree->Branch("D0KWeightErr",  &D0KWeightErr,  "D0KWeightErr/D");
  ...
  for(Int_t i=0; i<nEntriesMC; ++i) {
    ...
    if(D0pi_ID>0) {
      D0piBin = mcHist3->FindBin(B_D0_D0p_CM_P,0.5*log((B_D0_D0p_CM_P+B_D0_D0p_CM_PZ)/(B_D0_D0p_CM_P-B_D0_D0p_CM_PZ)),nTracks);
      D0KBin  = mcHist4->FindBin(B_D0_D0m_CM_P,0.5*log((B_D0_D0m_CM_P+B_D0_D0m_CM_PZ)/(B_D0_D0m_CM_P-B_D0_D0m_CM_PZ)),nTracks);

    } else if(D0pi_ID<0) {
      D0KBin  = mcHist4->FindBin(B_D0_D0p_CM_P,0.5*log((B_D0_D0p_CM_P+B_D0_D0p_CM_PZ)/(B_D0_D0p_CM_P-B_D0_D0p_CM_PZ)),nTracks);
      D0piBin = mcHist3->FindBin(B_D0_D0m_CM_P,0.5*log((B_D0_D0m_CM_P+B_D0_D0m_CM_PZ)/(B_D0_D0m_CM_P-B_D0_D0m_CM_PZ)),nTracks);
    ...
    D0piWeight = weightHist3->GetBinContent(D0piBin);
    D0KWeight  = weightHist4->GetBinContent(D0KBin );

    piWeightErr   = weightHist1->GetBinError(piBin  );
    KWeightErr    = weightHist2->GetBinError(KBin   );
    D0piWeightErr = weightHist3->GetBinError(D0piBin);
    D0KWeightErr  = weightHist4->GetBinError(D0KBin );
