void fit(TH1D* h, Double_t& sh, Double_t& sl, TString plotName="") {
   RooRealVar eff("eff","",0.95,1.15);
   RooDataHist * data = new RooDataHist("data", "", RooArgSet(eff), h);

   RooRealVar mean("mean","",1.05,0.95,1.15,"");
   //RooRealVar sigmaHi("sigmaHi","",0.01,0.003,0.1,"");
   RooRealVar sigmaLo("sigmaLo","",0.01,0.001,0.1,"");
...
void fitToyHists() {
   ...
   TFile* f = TFile::Open("trk_sq_Bd_toys.root");
   for(Int_t i=1; i<=nBins; ++i) {
	 ...
	 hName += j;
         TH1D* h = f->Get(hName);
	 TString pName = "latexToyFits/figs/";
	 pName += i;
	 pName += "_";
	 pName += j;
	 pName += ".pdf";
   ...
   TFile* outfile = TFile::Open("hists/trk_sq10_Bd_errToy_asym.root","recreate");
