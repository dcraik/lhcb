...
void l0h_Eff(Double_t NNmin=-0.80, Double_t NNmax=1.00){
	...
	TFile * file = TFile::Open("/data/lhcb/phrkbf/B2DKpi/JobSim08aMC/D2Kpi/Bd2D0Kpi/D2Kpi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2Kpi_addIMs_addMisIDIMs.root");
	...
	Double_t pi_et, K_et, Dpi_et, DK_et;
	Double_t pi_etAdd, K_etAdd, Dpi_etAdd, DK_etAdd;
	Int_t pi_HCAL_region, K_HCAL_region, Dpi_HCAL_region, DK_HCAL_region;
	Double_t pi_x, pi_y, K_x, K_y, Dpi_x, Dpi_y, DK_x, DK_y;
	...
	tree->SetBranchAddress("D0pi_L0Calo_HCAL_realET",      &Dpi_et);
        tree->SetBranchAddress("D0pi_L0Calo_HCAL_region",      &Dpi_HCAL_region);
	tree->SetBranchAddress("D0pi_L0Calo_HCAL_xProjection", &Dpi_x);
	tree->SetBranchAddress("D0pi_L0Calo_HCAL_yProjection", &Dpi_y);
	tree->SetBranchAddress("D0K_L0Calo_HCAL_realET",       &DK_et);
        tree->SetBranchAddress("D0K_L0Calo_HCAL_region",       &DK_HCAL_region);
	tree->SetBranchAddress("D0K_L0Calo_HCAL_xProjection",  &DK_x);
	tree->SetBranchAddress("D0K_L0Calo_HCAL_yProjection",  &DK_y);
	...
	tree->SetBranchAddress("D0pi_ID",                 &D0pi_ID);
		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0K_TRUEID)==321&&abs(D0pi_TRUEID)==211) {
			if(K_MC_MOTHER_KEY==pi_MC_MOTHER_KEY&&K_MC_MOTHER_KEY==D0K_MC_GD_MOTHER_KEY&&K_MC_MOTHER_KEY==D0pi_MC_GD_MOTHER_KEY) {
				if(l0_GlobalTIS==1) {
					...
					Int_t closestPair=0; //0 = piK, 1 = piDpi, 2 = piDK, 3 = KDpi, 4 = KDK, 5 = DpiDK
					...
					if((pi_x-Dpi_x)*(pi_x-Dpi_x) + (pi_y-Dpi_y)*(pi_y-Dpi_y) < shortestDistSq) {
						closestPair=1;
						regionA=pi_HCAL_region;
						regionB=Dpi_HCAL_region;
						shortestDistX=abs(pi_x-Dpi_x);
					 	shortestDistY=abs(pi_y-Dpi_y);
						shortestDistSq=(pi_x-Dpi_x)*(pi_x-Dpi_x) + (pi_y-Dpi_y)*(pi_y-Dpi_y);
					}
					if((pi_x-DK_x)*(pi_x-DK_x) + (pi_y-DK_y)*(pi_y-DK_y) < shortestDistSq) {
						closestPair=2;
						regionA=pi_HCAL_region;
						regionB=DK_HCAL_region;
						shortestDistX=abs(pi_x-DK_x);
					 	shortestDistY=abs(pi_y-DK_y);
						shortestDistSq=(pi_x-DK_x)*(pi_x-DK_x) + (pi_y-DK_y)*(pi_y-DK_y);
					}
					if((K_x-Dpi_x)*(K_x-Dpi_x) + (K_y-Dpi_y)*(K_y-Dpi_y) < shortestDistSq) {
						closestPair=3;
						regionA=K_HCAL_region;
						regionB=Dpi_HCAL_region;
						shortestDistX=abs(K_x-Dpi_x);
					 	shortestDistY=abs(K_y-Dpi_y);
						shortestDistSq=(K_x-Dpi_x)*(K_x-Dpi_x) + (K_y-Dpi_y)*(K_y-Dpi_y);
					}
					if((K_x-DK_x)*(K_x-DK_x) + (K_y-DK_y)*(K_y-DK_y) < shortestDistSq) {
						closestPair=4;
						regionA=K_HCAL_region;
						regionB=DK_HCAL_region;
						shortestDistX=abs(K_x-DK_x);
					 	shortestDistY=abs(K_y-DK_y);
						shortestDistSq=(K_x-DK_x)*(K_x-DK_x) + (K_y-DK_y)*(K_y-DK_y);
					}
					if((Dpi_x-DK_x)*(Dpi_x-DK_x) + (Dpi_y-DK_y)*(Dpi_y-DK_y) < shortestDistSq) {
						closestPair=5;
						regionA=Dpi_HCAL_region;
						regionB=DK_HCAL_region;
						shortestDistX=abs(Dpi_x-DK_x);
					 	shortestDistY=abs(Dpi_y-DK_y);
						shortestDistSq=(Dpi_x-DK_x)*(Dpi_x-DK_x) + (Dpi_y-DK_y)*(Dpi_y-DK_y);
					}

					Double_t cellSize = 999999.;//TODO should be 0 - was correct for KK and pipi
					...
					switch(closestPair) {
						case 0:
							...
							if(Polarity==-1) {
								...
								if(D0pi_ID>0) {
									if(Dpi_HCAL_region==0) h3 = pip_md_out;
									else h3 = pip_md_in;
									if(DK_HCAL_region==0) h4 = km_md_out;
									else h4 = km_md_in;
								} else {
									if(Dpi_HCAL_region==0) h3 = pim_md_out;
									else h3 = pim_md_in;
									if(DK_HCAL_region==0) h4 = kp_md_out;
									else h4 = kp_md_in;
								}
							} else {
								if(pi_ID>0) {
								...
								if(D0pi_ID>0) {
									if(Dpi_HCAL_region==0) h3 = pip_mu_out;
									else h3 = pip_mu_in;
									if(DK_HCAL_region==0) h4 = km_mu_out;
									else h4 = km_mu_in;
								} else {
									if(Dpi_HCAL_region==0) h3 = pim_mu_out;
									else h3 = pim_mu_in;
									if(DK_HCAL_region==0) h4 = kp_mu_out;
									else h4 = kp_mu_in;
								}
							}
							break;
						case 1:
							...
							if(Polarity==-1) {
								...
								if(D0pi_ID>0) {
									if(Dpi_HCAL_region==0) h2 = pip_md_out;
									else h2 = pip_md_in;
									if(DK_HCAL_region==0) h4 = km_md_out;
									else h4 = km_md_in;
								} else {
									if(Dpi_HCAL_region==0) h2 = pim_md_out;
									else h2 = pim_md_in;
									if(DK_HCAL_region==0) h4 = kp_md_out;
									else h4 = kp_md_in;
								}
								...
								if(D0pi_ID>0) {
									if(Dpi_HCAL_region==0) h2 = pip_mu_out;
									else h2 = pip_mu_in;
									if(DK_HCAL_region==0) h4 = km_mu_out;
									else h4 = km_mu_in;
								} else {
									if(Dpi_HCAL_region==0) h2 = pim_mu_out;
									else h2 = pim_mu_in;
									if(DK_HCAL_region==0) h4 = kp_mu_out;
									else h4 = kp_mu_in;
								}
						...
						case 2:
							...
							if(Polarity==-1) {
								...
								if(D0pi_ID>0) {
									...
									if(DK_HCAL_region==0) h2 = km_md_out;
									else h2 = km_md_in;
								} else {
									if(Dpi_HCAL_region==0) h3 = pim_md_out;
									else h3 = pim_md_in;
									if(DK_HCAL_region==0) h2 = kp_md_out;
									else h2 = kp_md_in;
								}
							} else {
								if(pi_ID>0) {
								...
								if(D0pi_ID>0) {
									...
									if(DK_HCAL_region==0) h2 = km_mu_out;
									else h2 = km_mu_in;
								} else {
									if(Dpi_HCAL_region==0) h3 = pim_mu_out;
									else h3 = pim_mu_in;
									if(DK_HCAL_region==0) h2 = kp_mu_out;
									else h2 = kp_mu_in;
								}
							}
							break;
						case 3:
							...
							if(Polarity==-1) {
								...
								if(D0pi_ID>0) {
									if(Dpi_HCAL_region==0) h1 = pip_md_out;
									else h1 = pip_md_in;
									if(DK_HCAL_region==0) h4 = km_md_out;
									else h4 = km_md_in;
								} else {
									if(Dpi_HCAL_region==0) h1 = pim_md_out;
									else h1 = pim_md_in;
									if(DK_HCAL_region==0) h4 = kp_md_out;
									else h4 = kp_md_in;
								}
								...
								if(D0pi_ID>0) {
									if(Dpi_HCAL_region==0) h1 = pip_mu_out;
									else h1 = pip_mu_in;
									if(DK_HCAL_region==0) h4 = km_mu_out;
									else h4 = km_mu_in;
								} else {
									if(Dpi_HCAL_region==0) h1 = pim_mu_out;
									else h1 = pim_mu_in;
									if(DK_HCAL_region==0) h4 = kp_mu_out;
									else h4 = kp_mu_in;
								}
						...
						case 4:
							...
							if(Polarity==-1) {
								...
								if(D0pi_ID>0) {
									...
									if(DK_HCAL_region==0) h1 = km_md_out;
									else h1 = km_md_in;
								} else {
									if(Dpi_HCAL_region==0) h3 = pim_md_out;
									else h3 = pim_md_in;
									if(DK_HCAL_region==0) h1 = kp_md_out;
									else h1 = kp_md_in;
								}
							} else {
								if(pi_ID>0) {
								...
								if(D0pi_ID>0) {
									...
									if(DK_HCAL_region==0) h1 = km_mu_out;
									else h1 = km_mu_in;
								} else {
									if(Dpi_HCAL_region==0) h3 = pim_mu_out;
									else h3 = pim_mu_in;
									if(DK_HCAL_region==0) h1 = kp_mu_out;
									else h1 = kp_mu_in;
								}
							}
							break;
...
//					if(combine) std::cout << "combining pair " << closestPair << std::endl;
//					else if(alpha>0.) std::cout << "sharing " << alpha*100 << "%% between pair" << closestPair << std::endl;
//					else std::cout << "no sharing" << std::endl;
					...
					if(combine) {
						...
						//std::cout << p12 << "\t" << sqrt(e12sq) << std::endl;
					...
					//std::cout << eff << "\t" << sqrt(err2) << "\t" << p12 << "\t" << sqrt(e12sq) << "\t" << p3 << "\t" << e3 << "\t" << p4 << "\t" << e4 << std::endl;
					//std::cout << eff << "\t" << 1.-(1.-p1)*(1.-p2)*(1.-p3)*(1.-p4) << std::endl;
					...
					hNoOverlap->Fill(  kinematics->getmPrime(), kinematics->getThetaPrime(), (1.-p1)*(1.-p2)*(1.-p3)*(1.-p4));
...
//	TString fileName("l0h_sq_Bs_effs.root");
//	outfile = new TFile(fileName,"RECREATE");
//	hEff->SetName("effData");
//	hEffSel->SetName("effMC");
//	hRatio->SetName("ratio");
//	hEff->Write();
//	hEffSel->Write();
//	hRatio->Write();
...
//	hEff->GetZaxis()->SetRangeUser(0.95,1.1);
	hEff->Draw("colz");
//	printLHCb("R","S");
...
//	hEffSel->GetZaxis()->SetRangeUser(0.95,1.1);
	hEffSel->Draw("colz");
//	printLHCb("R","S");
	TString sdpPlotName("plots/l0h_sq");
	sdpPlotName+=nbins;
...
//	hRatio->GetZaxis()->SetRangeUser(0.95,1.1);
	hRatio->Draw("colz");
//	printLHCb("R","S");
	TString sdpPlotName("plots/l0h_sq");
	sdpPlotName+=nbins;
...
//	hOverlapRatio->GetZaxis()->SetRangeUser(0.95,1.1);
...
//	printLHCb("R","S");
//	TString dpPlotName("plots/l0h_sq");
//	dpPlotName+=nbins;
