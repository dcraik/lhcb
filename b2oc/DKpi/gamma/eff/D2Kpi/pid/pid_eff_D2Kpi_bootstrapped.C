...
void pid_Eff(Int_t seed) {
	...
	TString fileName="D2Kpi/Bd2D0Kpi/D2Kpi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2Kpi_addIMs_addMisIDIMs";
		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0K_TRUEID)==321&&abs(D0pi_TRUEID)==211) {
			if(K_MC_MOTHER_KEY==pi_MC_MOTHER_KEY&&K_MC_MOTHER_KEY==D0K_MC_GD_MOTHER_KEY&&K_MC_MOTHER_KEY==D0pi_MC_GD_MOTHER_KEY) {
				...
				if(NOCM_K_ID>0) {
				   ...
				   binDpi = Dpi_hist->FindBin(D0_D0m_CM_P,D0_D0m_CM_PT,1.25*nTracks);
				   binDK  = DK_hist->FindBin(D0_D0p_CM_P,D0_D0p_CM_PT,1.25*nTracks);

				} else {
				   binPi  = pi_hist->FindBin(D0_p_CM_P,D0_p_CM_PT,1.25*nTracks);
				   binK   = K_hist->FindBin(D0_m_CM_P,D0_m_CM_PT,1.25*nTracks);
				   binDpi = Dpi_hist->FindBin(D0_D0p_CM_P,D0_D0p_CM_PT,1.25*nTracks);
				   binDK  = DK_hist->FindBin(D0_D0m_CM_P,D0_D0m_CM_PT,1.25*nTracks);
				}
