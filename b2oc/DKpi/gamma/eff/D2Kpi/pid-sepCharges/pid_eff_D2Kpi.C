		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0K_TRUEID)==321&&abs(D0pi_TRUEID)==211) {
			if(K_MC_MOTHER_KEY==pi_MC_MOTHER_KEY&&K_MC_MOTHER_KEY==D0K_MC_GD_MOTHER_KEY&&K_MC_MOTHER_KEY==D0pi_MC_GD_MOTHER_KEY) {
				...
				if(NOCM_K_ID>0) {
				   ...
				   binDpi = Dpim_hist->FindBin(D0_D0m_CM_P,D0_D0m_CM_PT,1.25*nTracks);
				   binDK  = DKp_hist->FindBin(D0_D0p_CM_P,D0_D0p_CM_PT,1.25*nTracks);
				   ...
				   DpiEff = Dpim_hist->GetBinContent(binDpi);
				   DpiErr = Dpim_hist->GetBinError(binDpi);

				   DKEff  = DKp_hist->GetBinContent(binDK);
				   DKErr  = DKp_hist->GetBinError(binDK);
				} else {
				   binPi  = pip_hist->FindBin(D0_p_CM_P,D0_p_CM_PT,1.25*nTracks);
				   binK   = Km_hist->FindBin(D0_m_CM_P,D0_m_CM_PT,1.25*nTracks);
				   binDpi = Dpip_hist->FindBin(D0_D0p_CM_P,D0_D0p_CM_PT,1.25*nTracks);
				   binDK  = DKm_hist->FindBin(D0_D0m_CM_P,D0_D0m_CM_PT,1.25*nTracks);
				   ...
				   DpiEff = Dpip_hist->GetBinContent(binDpi);
				   DpiErr = Dpip_hist->GetBinError(binDpi);

				   DKEff  = DKm_hist->GetBinContent(binDK);
				   DKErr  = DKm_hist->GetBinError(binDK);
				}
