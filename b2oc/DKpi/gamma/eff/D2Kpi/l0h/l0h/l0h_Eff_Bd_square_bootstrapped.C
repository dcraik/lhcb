...
void l0h_Eff(Int_t seed){
	...
	TFile * file = TFile::Open("/data/lhcb/phrkbf/B2DKpi/JobSim08aMC/Bd2DKpi/Bd2D0Kpi/Bd2DKpi_Bd2D0Kpi_P8_selBd_Dst25_noDPVetoes_NNf1_noPID_noD0PID_misIDIMs.root");

	TTree * tree = dynamic_cast<TTree*>( file->Get("DecayTree") );
	Int_t nEntries = tree->GetEntries();

	std::vector<Int_t> entries;
	TRandom3 rand(40000+seed);
	for(Int_t i=0; i<nEntries; ++i) {
           entries.push_back((Int_t)(rand.Rndm()*nEntries));
	}
	...
	tree->SetBranchAddress("pythia",                  &pythia);
...
//					else std::cout << "no sharing" << std::endl;
	...
	for (Int_t k=0; k<nbins; ++k) {
	   for (Int_t l=0;l<nbins;++l) {
	      Float_t temp2 = hErr2->GetBinContent(k+1,l+1);
	      hCorr->SetBinError(k+1,l+1,sqrt(temp2));
	   }
	}
	...
	TString fileName("bootstrapped/l0h_sq_Bd_effs");
	fileName += seed;
	fileName += ".root";
