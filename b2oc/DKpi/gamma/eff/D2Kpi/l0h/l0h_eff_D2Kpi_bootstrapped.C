	...
	for ( Int_t i(0); i < nEntries; ++i ) {
		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0K_TRUEID)==321&&abs(D0pi_TRUEID)==211) {
			if(K_MC_MOTHER_KEY==pi_MC_MOTHER_KEY&&K_MC_MOTHER_KEY==D0K_MC_GD_MOTHER_KEY&&K_MC_MOTHER_KEY==D0pi_MC_GD_MOTHER_KEY) {
				//use only TIS candidates to avoid bias towards TOS
				if(l0_GlobalTIS==1) {
					//MC behaviour gives denominator
					if(l0_HadronTOS==1) {
						hSel->Fill(kinematics->getmPrime(), kinematics->getThetaPrime(), 1.0);
					}
					eff     = 0.;

					//Get data behaviour (numerator) from lookup tables

					//treat closest pair together to account for most significant overlap
