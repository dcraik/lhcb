void fit(TH1D* h, Double_t& sh, Double_t& sl, TString plotName="") {
   RooRealVar eff("eff","",0.2,0.9);
   RooDataHist * data = new RooDataHist("data", "", RooArgSet(eff), h);

   RooRealVar mean("mean","",0.5,0.3,0.7,"");
   //RooRealVar sigmaHi("sigmaHi","",0.01,0.003,0.1,"");
   RooRealVar sigmaLo("sigmaLo","",0.01,0.003,0.1,"");
...
void fitBootstrappedHists() {
   ...
   TString inName("pid_bootstrapped.root");
   ...
   TString outname = "hists/pid_errStat_asym.root";
