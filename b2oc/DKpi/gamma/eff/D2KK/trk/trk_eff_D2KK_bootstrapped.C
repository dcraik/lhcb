...
void trk_Eff(Int_t seed){
	...
	for ( Int_t i(0); i < nEntries; ++i ) {
		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0m_TRUEID)==321&&abs(D0p_TRUEID)==321) {
	...
	TString outFileName("bootstrapped/trk_sq_Bd_effs");
	outFileName+=seed;
	outFileName+=".root";
