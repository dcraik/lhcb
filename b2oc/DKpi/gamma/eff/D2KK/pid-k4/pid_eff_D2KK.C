...
//	dpcut->Sumw2();
	...
	TH3F* K_hist        = (TH3F*)histfile->Get("K_ProbNNK * (1 - ProbNNpi ) > 0.4_All");
	...
	for ( Int_t i(0); i < nEntries; ++i ) {
		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0p_TRUEID)==321&&abs(D0m_TRUEID)==321) {
			if(K_MC_MOTHER_KEY==pi_MC_MOTHER_KEY&&K_MC_MOTHER_KEY==D0p_MC_GD_MOTHER_KEY&&K_MC_MOTHER_KEY==D0m_MC_GD_MOTHER_KEY) {
				...
				binDp  = DK_hist->FindBin(D0_D0p_CM_P,D0_D0p_CM_PT,1.25*nTracks);
				binDm  = DK_hist->FindBin(D0_D0m_CM_P,D0_D0m_CM_PT,1.25*nTracks);
				...
				DpEff = DK_hist->GetBinContent(binDp);
				DpErr = DK_hist->GetBinError(binDp);

				DmEff  = DK_hist->GetBinContent(binDm);
				DmErr  = DK_hist->GetBinError(binDm);
