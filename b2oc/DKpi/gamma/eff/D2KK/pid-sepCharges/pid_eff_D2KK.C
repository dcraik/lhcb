	...
	for ( Int_t i(0); i < nEntries; ++i ) {
		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0p_TRUEID)==321&&abs(D0m_TRUEID)==321) {
			if(K_MC_MOTHER_KEY==pi_MC_MOTHER_KEY&&K_MC_MOTHER_KEY==D0p_MC_GD_MOTHER_KEY&&K_MC_MOTHER_KEY==D0m_MC_GD_MOTHER_KEY) {
				...
				binDp  = DKp_hist->FindBin(D0_D0p_CM_P,D0_D0p_CM_PT,1.25*nTracks);
				binDm  = DKm_hist->FindBin(D0_D0m_CM_P,D0_D0m_CM_PT,1.25*nTracks);

				DpEff = DKp_hist->GetBinContent(binDp);
				DpErr = DKp_hist->GetBinError(binDp);

				DmEff  = DKm_hist->GetBinContent(binDm);
				DmErr  = DKm_hist->GetBinError(binDm);
...
//  pid_Eff("Sim08aMC","Bd2DKpi/Bd2D0Kpi/Bd2DKpi_Bd2D0Kpi_P8_selBd_noDPVetoes_NN","DecayTree");
  pid_Eff("Sim08aMC","D2KK/Bd2D0Kpi/D2KK_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2KK_addIMs_addMisIDIMs","DecayTree",-0.80, 1.00,"all");
//  pid_Eff("Sim08aMC","D2KK/Bd2D0Kpi/D2KK_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2KK_addIMs_addMisIDIMs","DecayTree",-0.80,-0.25,"a");
//  pid_Eff("Sim08aMC","D2KK/Bd2D0Kpi/D2KK_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2KK_addIMs_addMisIDIMs","DecayTree",-0.25, 0.23,"b");
//  pid_Eff("Sim08aMC","D2KK/Bd2D0Kpi/D2KK_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2KK_addIMs_addMisIDIMs","DecayTree", 0.23, 0.57,"c");
//  pid_Eff("Sim08aMC","D2KK/Bd2D0Kpi/D2KK_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2KK_addIMs_addMisIDIMs","DecayTree", 0.57, 0.74,"d");
//  pid_Eff("Sim08aMC","D2KK/Bd2D0Kpi/D2KK_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2KK_addIMs_addMisIDIMs","DecayTree", 0.74, 1.00,"e");
