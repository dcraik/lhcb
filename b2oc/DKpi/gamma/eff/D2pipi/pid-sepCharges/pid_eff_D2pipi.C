...
//	dpcut->Sumw2();
	...
	// Get Histograms
	TFile * histfile    = TFile::Open("/home/phrkbf/PIDCalibHists/new/pos/PerfHists_K_Strip20rX_MagBoth_DKpi_PPTN3_P_PT_nTracks.root");
	TH3F* Kp_hist        = (TH3F*)histfile->Get("K_ProbNNK * (1 - ProbNNpi ) > 0.3_All");
	TH3F* DKp_hist       = (TH3F*)histfile->Get("K_ProbNNK * (1 - ProbNNpi ) > 0.1_All");

	TFile * histfile2   = TFile::Open("/home/phrkbf/PIDCalibHists/new/pos/PerfHists_Pi_Strip20rX_MagBoth_DKpi_PPTN3_P_PT_nTracks.root");
	TH3F* pip_hist       = (TH3F*)histfile2->Get("Pi_ProbNNpi * (1 - ProbNNK ) > 0.2_All");
	TH3F* Dpip_hist      = (TH3F*)histfile2->Get("Pi_ProbNNpi * (1 - ProbNNK ) > 0.1_All");

	TFile * histfile3   = TFile::Open("/home/phrkbf/PIDCalibHists/new/neg/PerfHists_K_Strip20rX_MagBoth_DKpi_PPTN3_P_PT_nTracks.root");
	TH3F* Km_hist        = (TH3F*)histfile3->Get("K_ProbNNK * (1 - ProbNNpi ) > 0.3_All");
	TH3F* DKm_hist       = (TH3F*)histfile3->Get("K_ProbNNK * (1 - ProbNNpi ) > 0.1_All");

	TFile * histfile4   = TFile::Open("/home/phrkbf/PIDCalibHists/new/neg/PerfHists_Pi_Strip20rX_MagBoth_DKpi_PPTN3_P_PT_nTracks.root");
	TH3F* pim_hist       = (TH3F*)histfile4->Get("Pi_ProbNNpi * (1 - ProbNNK ) > 0.2_All");
	TH3F* Dpim_hist      = (TH3F*)histfile4->Get("Pi_ProbNNpi * (1 - ProbNNK ) > 0.1_All");
		...
		if(abs(B_TRUEID)==511&&abs(D_TRUEID)==421&&abs(K_TRUEID)==321&&abs(pi_TRUEID)==211&&abs(D0p_TRUEID)==211&&abs(D0m_TRUEID)==211) {
			if(K_MC_MOTHER_KEY==pi_MC_MOTHER_KEY&&K_MC_MOTHER_KEY==D0p_MC_GD_MOTHER_KEY&&K_MC_MOTHER_KEY==D0m_MC_GD_MOTHER_KEY) {
				...
				D0_D0m_CM_PT = sqrt(D0_D0m_CM_PX*D0_D0m_CM_PX + D0_D0m_CM_PY*D0_D0m_CM_PY);
					...
					// Get eff for K track
				if(NOCM_K_ID>0) {
				   binPi  = pim_hist->FindBin(D0_m_CM_P,D0_m_CM_PT,1.25*nTracks);
				   binK   = Kp_hist->FindBin(D0_p_CM_P,D0_p_CM_PT,1.25*nTracks);

				   piEff  = pim_hist->GetBinContent(binPi);
				   piErr  = pim_hist->GetBinError(binPi);

				   KEff   = Kp_hist->GetBinContent(binK);
				   KErr   = Kp_hist->GetBinError(binK);
				} else {
				   binPi  = pip_hist->FindBin(D0_p_CM_P,D0_p_CM_PT,1.25*nTracks);
				   binK   = Km_hist->FindBin(D0_m_CM_P,D0_m_CM_PT,1.25*nTracks);

				   piEff  = pip_hist->GetBinContent(binPi);
				   piErr  = pip_hist->GetBinError(binPi);

				   KEff   = Km_hist->GetBinContent(binK);
				   KErr   = Km_hist->GetBinError(binK);
				}
				binDp  = Dpip_hist->FindBin(D0_D0p_CM_P,D0_D0p_CM_PT,1.25*nTracks);
				binDm  = Dpim_hist->FindBin(D0_D0m_CM_P,D0_D0m_CM_PT,1.25*nTracks);

				DpEff = Dpip_hist->GetBinContent(binDp);
				DpErr = Dpip_hist->GetBinError(binDp);

				DmEff  = Dpim_hist->GetBinContent(binDm);
				DmErr  = Dpim_hist->GetBinError(binDm);


				weight = piEff * KEff * DpEff * DmEff;
				err2   = piErr*piErr*KEff*KEff*DpEff*DpEff*DmEff*DmEff +
					 piEff*piEff*KErr*KErr*DpEff*DpEff*DmEff*DmEff +
					 piEff*piEff*KEff*KEff*DpErr*DpErr*DmEff*DmEff +
					 piEff*piEff*KEff*KEff*DpEff*DpEff*DmErr*DmErr;
...
//  pid_Eff("Sim08aMC","Bd2DKpi/Bd2D0Kpi/Bd2DKpi_Bd2D0Kpi_P8_selBd_noDPVetoes_NN","DecayTree");
  pid_Eff("Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree",-0.80, 1.00,"all");
//  pid_Eff("Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree",-0.80,-0.25,"a");
//  pid_Eff("Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree",-0.25, 0.23,"b");
//  pid_Eff("Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree", 0.23, 0.57,"c");
//  pid_Eff("Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree", 0.57, 0.74,"d");
//  pid_Eff("Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree", 0.74, 1.00,"e");
