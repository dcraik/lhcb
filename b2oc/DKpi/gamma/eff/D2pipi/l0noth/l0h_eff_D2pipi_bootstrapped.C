...
void l0h_Eff(Int_t seed){
	...
	TFile * file = TFile::Open("/data/lhcb/phrkbf/B2DKpi/JobSim08aMC/D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs.root");
					...
					switch(closestPair) {
						...
						case 1:
							...
							if(Polarity==-1) {
								...
								if(Dm_HCAL_region==0) h2 = pim_mu_out;
								else h2 = pim_mu_in;
								if(Dp_HCAL_region==0) h4 = pip_mu_out;
								else h4 = pip_mu_in;
							}
						...
						case 2:
							...
							if(Polarity==-1) {
								...
								if(Dp_HCAL_region==0) h2 = pip_md_out;
								else h2 = pip_md_in;
							} else {
								if(pi_ID>0) {
									if(pi_HCAL_region==0) h1 = pip_mu_out;
								...
								if(Dp_HCAL_region==0) h2 = pip_mu_out;
								else h2 = pip_mu_in;
							}
							break;
						case 3:
							...
							if(Polarity==-1) {
								...
								if(Dm_HCAL_region==0) h1 = pim_md_out;
								else h1 = pim_md_in;
								if(Dp_HCAL_region==0) h4 = pip_md_out;
								else h4 = pip_md_in;
							} else {
								...
								if(Dm_HCAL_region==0) h1 = pim_mu_out;
								else h1 = pim_mu_in;
								if(Dp_HCAL_region==0) h4 = pip_mu_out;
								else h4 = pip_mu_in;
							}
						...
						case 4:
							...
							if(Polarity==-1) {
								...
								if(Dp_HCAL_region==0) h1 = pip_md_out;
								else h1 = pip_md_in;
							} else {
								if(pi_ID>0) {
								...
								if(Dp_HCAL_region==0) h1 = pip_mu_out;
								else h1 = pip_mu_in;
							}
							break;
						case 5:
