{
  ...
  TFile* dataFile    = TFile::Open("/data/lhcb/phrkbf/B2DKpi/Job2012_B2DKpi_D2pipi_Scaled/B2D0Kpi_D02pipi_selBd_Dsignal_Dst25_vetoes_PID3_NND2pipi_addIMs.root");
  TFile* mcFile      = TFile::Open("/data/lhcb/phrkbf/B2DKpi/JobSim08aMC/D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs.root");
...
//      std::cout << stepSize << "\t" << cutVal << "\t" << nBelow << "\t" << perBin*(i+1) << endl;
  ...
  for(Int_t i=0; i<nEntriesMC; ++i) {
    ...
    if(abs(B_TRUEID)!=511||abs(D_TRUEID)!=421||abs(K_TRUEID)!=321||abs(pi_TRUEID)!=211||abs(D0p_TRUEID)!=211||abs(D0m_TRUEID)!=211) continue;
  ...
  TFile * newFile = TFile::Open("D2pipi_Bd2D0Kpi_trackWeight.root","recreate");
