...
void sel_Eff(TString jobName, TString fileName, TString treeName, TString jobName2, TString fileName2, TString treeName2, Double_t NNmin=-0.8, Double_t NNmax=1.0, TString binLabel="",
	...
	kinematics = new LauKinematics(mD,mK,mPi,mB,true);
...
//	tree->SetBranchAddress("D_TRUEID",&D_TRUEID);
	...
	for ( Int_t i(0); i < nEntries; ++i ) {
		...
		++total;
		if(!kinematics->withinDPLimits(D0h2.M2()/1000000,h1h2.M2()/1000000)) {
...
//                  cout << "event accepted at " << D0h1.M2() << ", " << D0h2.M2() << endl;
                ...
                kinematics->updateKinematics(D0h2.M2()/1000000,h1h2.M2()/1000000);
...
//	tree2->SetBranchAddress("Matched",&Matched);
	...
	tree2->SetBranchAddress("D0p_TRUEID",              &D0p_TRUEID);
	tree2->SetBranchAddress("D0m_TRUEID",              &D0m_TRUEID);
	tree2->SetBranchAddress("K_TRUEID",                &K_TRUEID);
	tree2->SetBranchAddress("pi_TRUEID",               &pi_TRUEID);

	tree2->SetBranchAddress("D0p_MC_GD_MOTHER_KEY",    &D0p_MC_GD_MOTHER_KEY);
	tree2->SetBranchAddress("D0m_MC_GD_MOTHER_KEY",    &D0m_MC_GD_MOTHER_KEY);
...
//		++total;
		if(!kinematics->withinDPLimits(Bd_CM_m23Sq,Bd_CM_m12Sq)) {
...
//                  cout << "event accepted at " << D0h1.M2() << ", " << D0h2.M2() << endl;
	        ...
	        kinematics->updateKinematics(Bd_CM_m23Sq,Bd_CM_m12Sq);
...
//	cout << counter2<< " " << sum2<< " " << 100*sum2/counter2 << "%" << endl;
	...
	TString fname = "hists/seltrig_sq";
	fname += nbins;
	//fname += "_"; fname += NNmin1k; fname += "_"; fname += NNmax1k;
	fname += "_NN"; fname += binLabel;
	fname += "_all.root";
...
//  gROOT->ProcessLine(".L DPKinematics/LauKinematics.cc");
  ...
  TColor::CreateGradientColorTable(NRGBs, stops, red, green, blue, NCont);
  gStyle->SetNumberContours(NCont);
  sel_Eff("Sim08aMCEff","D2pipi/Bd2DKpi_truth","Bd2KpiD0/tupleMCTruth","Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree", -0.80,  1.00,"all");
  sel_Eff("Sim08aMCEff","D2pipi/Bd2DKpi_truth","Bd2KpiD0/tupleMCTruth","Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree", -0.80, -0.25,"a");
  sel_Eff("Sim08aMCEff","D2pipi/Bd2DKpi_truth","Bd2KpiD0/tupleMCTruth","Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree", -0.25,  0.23,"b");
  sel_Eff("Sim08aMCEff","D2pipi/Bd2DKpi_truth","Bd2KpiD0/tupleMCTruth","Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree",  0.23,  0.57,"c");
  sel_Eff("Sim08aMCEff","D2pipi/Bd2DKpi_truth","Bd2KpiD0/tupleMCTruth","Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree",  0.57,  0.74,"d");
  sel_Eff("Sim08aMCEff","D2pipi/Bd2DKpi_truth","Bd2KpiD0/tupleMCTruth","Sim08aMC","D2pipi/Bd2D0Kpi/D2pipi_Bd2D0Kpi_selBd_Dsignal_vetoes_noDPV_NND2pipi_addIMs_addMisIDIMs","DecayTree",  0.74,  1.00,"e");
}
