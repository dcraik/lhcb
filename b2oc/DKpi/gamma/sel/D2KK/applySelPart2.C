...
//	Double_t mDmK(0.), mDmKpi(0.), mDpK(0.), mDpKpi(0.), mKpi(0.);
	Double_t mD0K(0.);

//	Bool_t pi_isMuon(0.), D0p_isMuon(0.), D0m_isMuon(0.);

	TFile* file = TFile::Open("B2D0Kpi_D02KK_selBd_Dsidebands_NND2KK_addIMs.root");
...
//	tree->SetBranchAddress("D0m_isMuon"  ,     &D0m_isMuon    );
	...
	TFile* newfile = new TFile("B2D0Kpi_D02KK_selBd_Dsidebands_vetoes_PID3_NND2KK_addIMs.root","recreate");
	...
	for(Int_t i=0; i<n; ++i) {
		...
		if(K_ProbNNk*(1-K_ProbNNpi)>0.3 && pi_ProbNNpi*(1-pi_ProbNNk)>0.2 && D0p_ProbNNk*(1-D0p_ProbNNpi)>0.1 && D0m_ProbNNk*(1-D0m_ProbNNpi)>0.1) {//PID
