void makeXCheckTuples(TString xcheck) {
	TFile* file = TFile::Open("B2D0Kpi_D02KK_selBd_Dsignal_Dst25_vetoes_PID3_NND2KK_addIMs.root");
	TTree* tree = (TTree*)file->Get("DecayTree");

	TFile* newfileA = new TFile("B2D0Kpi_D02KK_selBd_Dsignal_Dst25_vetoes_PID3_NND2KK_addIMs_Bd25_a_"+xcheck+".root","recreate");
	TTree* newtreeA = new TTree("DecayTree","DecayTree");

	TFile* newfileB = new TFile("B2D0Kpi_D02KK_selBd_Dsignal_Dst25_vetoes_PID3_NND2KK_addIMs_Bd25_b_"+xcheck+".root","recreate");
	TTree* newtreeB = new TTree("DecayTree","DecayTree");

	TFile* newfileC = new TFile("B2D0Kpi_D02KK_selBd_Dsignal_Dst25_vetoes_PID3_NND2KK_addIMs_Bd25_c_"+xcheck+".root","recreate");
	TTree* newtreeC = new TTree("DecayTree","DecayTree");

	TFile* newfileD = new TFile("B2D0Kpi_D02KK_selBd_Dsignal_Dst25_vetoes_PID3_NND2KK_addIMs_Bd25_d_"+xcheck+".root","recreate");
	TTree* newtreeD = new TTree("DecayTree","DecayTree");

	TFile* newfileE = new TFile("B2D0Kpi_D02KK_selBd_Dsignal_Dst25_vetoes_PID3_NND2KK_addIMs_Bd25_e_"+xcheck+".root","recreate");
	TTree* newtreeE = new TTree("DecayTree","DecayTree");

	//Double_t mBd(5278.53), sBd(12.65);
	Double_t mBd(5278.67), sBd(12.72);
	...
	Short_t Polarity(0);
	Double_t dataset(0);
	Bool_t B_L0Global_TIS(0);
	Bool_t B_L0HadronDecision_TOS(0);
	Double_t K_ProbNNk(0.), K_ProbNNpi(0.);
	...
	for (Int_t i=0; i<nentries;++i) {
		...
		if(xcheck=="MD"  && Polarity!=-1) continue;
		if(xcheck=="MU"  && Polarity!=1) continue;
		if(xcheck=="11"  && dataset!=11) continue;
		if(xcheck=="12"  && dataset!=12) continue;
		if(xcheck=="TIS" && B_L0Global_TIS!=1) continue;
		if(xcheck=="TOS" && B_L0HadronDecision_TOS!=1) continue;
		if(xcheck=="K4"  && K_ProbNNk*(1-K_ProbNNpi)<0.4) continue;
		...
		if(B_D0_B_CM_M > mBd - 2.5*sBd && B_D0_B_CM_M < mBd + 2.5*sBd) {
			...
			if(NN>-0.80 && NN<=-0.08) newtreeA->Fill();
			if(NN>-0.08 && NN<= 0.43) newtreeB->Fill();
			if(NN> 0.43 && NN<= 0.59) newtreeC->Fill();
			if(NN> 0.59 && NN<= 0.67) newtreeD->Fill();
			if(NN> 0.67 && NN<= 1.00) newtreeE->Fill();
		}
	}
	newtreeA->AutoSave();
	newfileA->Close();
...
void makeXCheckTuples() {
	makeXCheckTuples("MD");
	makeXCheckTuples("MU");
	makeXCheckTuples("11");
	makeXCheckTuples("12");
	makeXCheckTuples("TIS");
	makeXCheckTuples("TOS");
	makeXCheckTuples("K4");
}
