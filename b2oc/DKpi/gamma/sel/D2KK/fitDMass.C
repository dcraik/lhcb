...
TFile * file = TFile::Open("/data/lhcb/phrkbf/B2DKpi/Job2012_B2DKpi_D2KK_Scaled/B2D0Kpi_D02KK_selBd_Dsidebands_D50_DK52_PID3_NND2KK_addIMs.root");
TTree * DecayTree = dynamic_cast<TTree*>(file->Get("DecayTree"));

RooRealVar D_M("D_M","; m(KK) (MeV/c^{2}); Candidates / 0.9 MeV/c^{2}",  1770,1960);
...
// D->Kpi Gaussian
RooRealVar Kpimean("Kpimean","",1964.,1914.,2014.,"MeV/c^{2}");
RooRealVar Kpisigma("#Kpisigma","",20.,0.0,50.0,"MeV/c^{2}");
//RooRealVar Kpiratio("Kpiratio","",2.,1.0,3.0);
//RooProduct Kpisigma2("#Kpisigma2","",RooArgSet(Kpisigma,Kpiratio));
//RooRealVar Kpifrac("Kpifrac","",0.7,0.0,1.0);

RooGaussian Kpi( "Kpi", "", D_M, Kpimean, Kpisigma );
//RooGaussian Kpi2( "Kpi2", "", D_M, Kpimean, Kpisigma2 );
//RooAddPdf Kpi("Kpi","",RooArgList(Kpi1,Kpi2),RooArgList(Kpifrac));

// D->KKpi0 Gaussian
RooRealVar KKpimean("KKpimean","",1764.,1714.,1814.,"MeV/c^{2}");
RooRealVar KKpisigma("KKpisigma","",30.,0.0,100.0,"MeV/c^{2}");
//RooRealVar KKpiratio("KKpiratio","",2.,1.0,3.0);
//RooProduct KKpisigma2("KKpisigma2","",RooArgSet(KKpisigma,KKpiratio));
//RooRealVar KKpifrac("KKpifrac","",0.7,0.0,1.0);

RooGaussian KKpi( "KKpi", "", D_M, KKpimean, KKpisigma );
//RooGaussian KKpi2( "KKpi2", "", D_M, KKpimean, KKpisigma2 );
//RooAddPdf KKpi("KKpi","",RooArgList(KKpi1,KKpi2),RooArgList(KKpifrac));

// Number of signal & background events
RooRealVar nsig("nsig","", 10000.,-1000,30000,"Events");
RooRealVar nbkg("nbkg","", 15000.,-1000,30000,"Events");
RooRealVar nKpi("nKpi","", 10000.,-1000,30000,"Events");
RooRealVar nKKpi("nKpi","",10000.,-1000,30000,"Events");
...
D_Plot->GetXaxis()->SetTitle("m(KK) (MeV/c^{2})");
...
full_RF_PDF->plotOn(D_Plot, RooFit::Components("KKpi"),  RooFit::LineStyle(kDashed),RooFit::LineColor(kGreen));
