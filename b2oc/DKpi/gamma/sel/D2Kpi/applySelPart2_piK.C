void applySelPart2_piK() {
	...
	Double_t mKpiDK(0.), mKpi(0.), mKD(0.);

	Bool_t pi_isMuon(0.), D0pi_isMuon(0.);

	TFile* file = TFile::Open("B2D0Kpi_D02piK_selBd_Dsidebands_NND2Kpi_addIMs.root");
	...
	tree->SetBranchAddress("mKpiDK" ,          &mKpiDK        );
	...
	TFile* newfile = new TFile("B2D0Kpi_D02piK_selBd_Dsidebands_vetoes_PID3_NND2Kpi_addIMs.root","recreate");
	...
	for(Int_t i=0; i<n; ++i) {
		...
		if(K_ProbNNk*(1-K_ProbNNpi)>0.3 && pi_ProbNNpi*(1-pi_ProbNNk)>0.2 && D0K_ProbNNk*(1-D0K_ProbNNpi)>0.1 && D0pi_ProbNNpi*(1-D0pi_ProbNNk)>0.1) {//PID
			if((mKpi<1840||mKpi>1890)&&(mKpiDK<1940||mKpiDK>1990)) {//wrong D vetoes
